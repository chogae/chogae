...
객체생성(document.body, "가글미러컨테이너", "사백", `테두리`, "세로", "내부", "숨기기");

//가글미러화면함수
async function 가글미러화면() {
    try {
        화면잠금(``);
        가글화면전환(가글미러컨테이너);
        ...
        객체생성(가글미러컨테이너, "가글미러설명", "가로꽉", "밑줄", `여백`);
        가글미러설명.innerHTML = `
        거울 속 자신과 마주하세요<br>
        ...
        if (결과.성공) {
            ...
            객체생성(가글미러컨테이너, `미러박스`, `세로`, `테두리`, `가로꽉`, `여백`);
            객체생성(미러박스, `미러1`, `가로꽉`, `밑줄`, `여백`);
            미러1.innerHTML = `??? (Lv.${유저.스탯.레벨})`;

            객체생성(가글미러컨테이너, `미러도전`, ``, `테두리`, `버튼`, `여백`);
            미러도전.innerHTML = `도전`;

            객체생성(가글미러컨테이너, `미러정보`, "", "", "가로꽉", `세로`);
...
가글미러.onclick = async () => {
    try {
        if (가글미러컨테이너.style.display === "none") {
            가글미러화면();
        }
    } catch (error) {
