...
async function 가글스킬화면() {
    try {
        ...
        for (const i in 스킬모음) {
            ...
            window[`${i}스킬이름`].innerHTML = `${스킬모음[i].이름} +${스킬레벨}`;
        ...
        try {
            ...
            if (결과.성공) {
                ...
                for (const a in 장비모음) {
                    ...
                    <span>${장비모음[a][0]}</span>
                    <span>[${장비모음[a][1]}] ${장비모음[a][2]} +${장비모음[a][3]}</span>
                    `;
                ...
                for (const a in 스킬모음) {
                    if (a < 5) {
                        ...
                        window[`${a}스킬`].innerHTML = `${스킬모음[a].이름} +${유저.스탯.스킬[유저.스탯.프리셋][a]}`;
                            ...
                            for (const i in 결과.전투결과.전투로그) {
                                ...
                                for (const j in 결과.전투결과.전투로그[i]) {
                                    ...
                                    if (Number(j) === 결과.전투결과.전투로그[i].length - 1) {
                                        ...
                                        객체생성(window[`${i}내정보상대정보`], `${i}상대정보`, "플렉스일", "테두리", "세로", "여백");
                                        객체생성(window[`${i}상대정보`], `${i}상대이름`, "", "", "", "여백");
                                        window[`${i}상대이름`].innerHTML = 결과.마신.스탯.닉네임;
                                        객체생성(window[`${i}상대정보`], `${i}상대체력`, "", "가로꽉", "", "작게");
                                        window[`${i}상대체력`].innerHTML = `${숫자로만(결과.전투결과.전투로그[i][j].상대남은체력)}/${숫자로만(결과.마신.스탯.최종체력)}`;
                                        window[`${i}상대체력`].style.backgroundColor = 색상맵[`진빨`];
                                        window[`${i}상대체력`].style.width = `${100 * 결과.전투결과.전투로그[i][j].상대남은체력 / 결과.마신.스탯.최종체력}%`;
                                        window[`${i}상대정보`].style.border = `1px solid ${색상맵[`중빨`]}`;
        ...
        try {
            ...
            if (결과.성공) {
                ...
                for (const i in 모든수다) {
                    ...
                    객체생성(window[`광장${광장id}수다박스`], `광장${광장id}수다`, "", "", "여백");
