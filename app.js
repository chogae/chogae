...
객체생성(document.body, "가글마녀상컨테이너", "사백", "테두리", "세로", "내부", "숨기기");

//가글마녀상화면함수
async function 가글마녀상화면() {
    try {
        화면잠금(``);
        가글화면전환(가글마녀상컨테이너);
        ...
        객체생성(가글마녀상컨테이너, "가글마녀상설명", "가로꽉", "밑줄", "여백");
        가글마녀상설명.innerHTML = `
        악마성 한 층을 공략할때마다 물품이 업그레이드됩니다<br>
        현재 악마성 ${유저.스탯.최고층}층의 마녀상입니다<br>
        구현중입니다<br>
        `;

        객체생성(가글마녀상컨테이너, "마녀상박스", "가로꽉", "세로", "");
...
가글마녀상.onclick = async () => {
    try {
        if (가글마녀상컨테이너.style.display === "none") {
            가글마녀상화면();
        }
    } catch (error) {
